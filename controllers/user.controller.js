...
export const getAllUsersInfo = async (req, res) => {
  ...
  try {
    ...
    const resUsers = await User.find(filterQuery, { password: 0 })
                              .skip(skip)
                              .limit(limit)
                              .exec();
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (req.body.password) {
      ...
      if (decryptedOldPass !== req.body.currentPass) return res.status(401).json({ error: "Old password does'nt matched!!" });
